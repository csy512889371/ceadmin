...
import {showUpdatePageForUnifyDepartment, updateForUnifyDepartment} from '../actions';
import {UPDATE_FOR_UNIFY_DEPARTMENT_FETCH, UPDATE_FOR_UNIFY_DEPARTMENT_SUCCESS, UPDATE_FOR_UNIFY_DEPARTMENT_ERROR} from '../actionTypes';
...
const UpdateForm = ({type, res, form, handleSubmit, hideUpdatePage}) => {
    ...
    switch (type){
        case UPDATE_FOR_UNIFY_DEPARTMENT_FETCH:
            showMessage= 'none';
            message= '';
            messageType= '';
            loading = true;
            break;
        case UPDATE_FOR_UNIFY_DEPARTMENT_SUCCESS:
            showMessage= 'block';
            message= '保存成功';
            messageType= 'success';
            loading = false;
            break;
        case UPDATE_FOR_UNIFY_DEPARTMENT_ERROR:
            showMessage= 'block';
            message= '保存失败';
            messageType= 'error';
            loading = false;
...
const mapStateToProps = (state) => {
    const {updateType, res} = state.uumsUnifyDepartment;
    return {
        type: updateType,
        res: res,
    }
...
const mapDispatchToProps = (dispatch, ownProps) => {
    ...
    const hideUpdatePage = () => {
        dispatch(showUpdatePageForUnifyDepartment(false));
    };
    ...
    const handleSubmit = (e, id, orgId) => {
        ...
        ownProps.form.validateFieldsAndScroll((err, values) => {
            if (!err) {
                ...
                dispatch(updateForUnifyDepartment(values));
            }
        });
    };
