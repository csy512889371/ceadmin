...
import {init, checkKeysForResourceStructure, searchForResourceStructure, expandKeysForResourceStructure, findResourceStructureInTree, changeCheckableForResourceStruture, findAuthedForResourceStructure} from '../actions';
...
const ResourceStructure = ({uumsUnifyResourceStructure, onSelect, onCheck, onChange, generateList, onExpand}) => {
        const {type, message, expandedKeys, autoExpandParent, searchValue, checkedKeys, parentAuthedKeys, checkable, forbiddenKeys} = uumsUnifyResourceStructure;
        ...
        const loop = (data) => data.map((item) => {
            ...
            switch (type) {
                case 'APP':
                    typeHtml = <Tag color="#108ee9">系统</Tag>;
                    break;
                case 'MENU':
                    if(forbiddenKeys.find((value) => {return value === id})){
                        forbiddenTag = <Tag color="red">禁</Tag>;
                    }
                    typeHtml = <Tag color="#87d068">菜单</Tag>;
                    break;
                case 'CONTROLLER':
                    if(forbiddenKeys.find((value) => {return value === id})){
                        forbiddenTag = <Tag color="red">禁</Tag>;
                    }
                    typeHtml = <Tag color="#2db7f5">请求</Tag>;
                    break;
                default:
                    break;
            }
...
const mapDispatchToProps = (dispatch) => {
    ...
    const getData = () => {
        ...
        dispatch(unifyManageOrder());
        dispatch(unitInfoActions.changeForUnitInfo());
    };
