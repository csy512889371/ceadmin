...
function* fetchUpdate(data) {
    ...
    try {
        ...
        if (result.success) {
            const desc = '更新角色->【ID：'+orgRole.id+'，名称：'+orgRole.name+'，标识符：'
                +orgRole.sn+'，所属机构：'+orgRole.orgId+'】';
            log.saveAction(api.orgRole.relativePath.update, desc, new Date().getTime()-startTime, 200);
            const {name} = result.data;
