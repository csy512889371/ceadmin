...
const AppTable = ({uumsApp, appData, refresh}) => {
    const {type, params, message} = uumsApp;
    let dataSource = [];
    let pagination = {};
    switch (type) {
...
const mapStateToProps = (state) => {
    return {
        uumsApp: selectVisibleAppPage(state),
    };
};

const mapDispatchToProps = (dispatch) => {

    const appData = (uumsApp) => {
        const result = uumsApp.data;
        const success = result.success;
        if (success) {
            const {params} = uumsApp;
