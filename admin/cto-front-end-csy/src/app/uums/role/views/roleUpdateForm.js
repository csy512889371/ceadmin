...
import AppSelect from '../../resource/auth/views/appSelect';

const FormItem = Form.Item;

const {update} = api.role;
        ...
        this.props.form.validateFieldsAndScroll((err, values) => {
            if (!err) {
                ...
                update(values).then((res) => res.json())
                    .then((res) => {
                        if (res.success) {
                            const desc = '更新系统角色->【ID：' + values.id + '，名称：' + values.name + '，标识符：' + values.sn
                                + '，所属应用系统：' + values.appId + '】';
                            log.saveAction(api.role.relativePath.update, desc, new Date().getTime() - startTime, 200);
                            this.setState({
                                showMessage: 'block',
                                message: '保存成功',
                                messageType: 'success',
